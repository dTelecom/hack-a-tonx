...
export const useMediaConstraints = (initialConstraints, audioEnabled = true, videoEnabled = true) => {
  ...
  const defaultConstraints = useMemo(() => ({audio: true, video: videoConstraints}), [videoConstraints])
  ...
  const onDeviceChange = useCallback((type, deviceId) => {
    ...
    return newState
  }, [constraintsState, enabledState, getConstraints])

  const onMediaToggle = useCallback((type) => {
    setEnabledState(prev => ({...prev, [type]: !prev[type]}))
  }, [])

  const constraints = useMemo(() => getConstraints(constraintsState, enabledState), [getConstraints, constraintsState, enabledState])
