...
import { Address, Cell, fromNano, internal, OpenedContract, toNano } from "ton";
import { TonClient, WalletContractV4, contractAddress, SendMode } from "ton";
...
async function main() {
  ...
  const walletContract = client.open(WalletContractV4.create({ workchain, publicKey: walletKey.publicKey }));
  ...
  for (const rootContract of rootContracts) {
    ...
    const initDataCell = deployInitScript.initData(walletContract.address) as Cell;
    ...
    const newContractAddress = contractAddress(workchain, { code: initCodeCell, data: initDataCell });
    ...
    if (await client.isContractDeployed(newContractAddress)) {
      ...
      await performPostDeploymentTest(client, rootContract, deployInitScript, walletContract, walletKey.secretKey, newContractAddress);
      continue;
    }
    ...
    console.log(` - Block explorer link: https://${process.env.TESTNET ? "test." : ""}tonwhales.com/explorer/address/${newContractAddress.toString()}`);
...
async function performPostDeploymentTest(client: TonClient, rootContract: string, deployInitScript: any, walletContract: OpenedContract<WalletContractV4>, secretKey: Buffer, newContractAddress: Address) {
  ...
  await deployInitScript.postDeployTest(client, walletContract, secretKey, newContractAddress);
}
