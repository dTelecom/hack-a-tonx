...
func (c *MasterContract) SendCreateNode(via *wallet.Wallet, nodeHost string) error {

	body := cell.BeginCell().
		MustStoreUInt(opMasterCreateNode, 32).
		MustStoreUInt(0, 64).
		MustStoreUInt(uint64(len(nodeHost)), 8).
		MustStoreSlice([]byte(nodeHost), uint(len(nodeHost))*8).
		EndCell()
...
func (c *MasterContract) GetNodeHosts() ([]string, error) {
	...
	for cur := res.AsTuple()[0]; cur != nil; {
		tuple := cur.([]any)
		hosts = append(hosts, tuple[0].(*cell.Slice).MustLoadStringSnake())
		cur = tuple[1]
	}
	return hosts, nil
}
