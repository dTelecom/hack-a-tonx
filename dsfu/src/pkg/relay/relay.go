...
const (
	...
	signalerReadyEvent   = "ion_relay_ready"
)
...
func NewPeer(meta PeerMeta, conf *PeerConfig) (*Peer, error) {
	...
	sctp.OnDataChannel(func(channel *webrtc.DataChannel) {
		if channel.Label() == signalerLabel {
			...
			channel.OnOpen(func() {
				log.Printf("HERE channel.OnOpen")
				if f := p.onReady.Load(); f != nil {
					f.(func())()
				}
				log.Printf("HERE EMIT")
				p.Emit(signalerReadyEvent, nil)
			})
...
func (p *Peer) Offer(signalFn func(meta PeerMeta, signal []byte) ([]byte, error)) error {
	...
	p.signalingDC.OnOpen(func() {
		log.Printf("HERE p.signalingDC 1")
		// if f := p.onReady.Load(); f != nil {
		// 	f.(func())()
		// }
		log.Printf("HERE p.signalingDC 2")
	})
...
func (p *Peer) handleRequest(msg webrtc.DataChannelMessage) {
	...
	if mr.Event == signalerReadyEvent {
		log.Printf("HERE signalerReadyEvent 1")
		if f := p.onReady.Load(); f != nil {
			go f.(func())()
		}
		log.Printf("HERE signalerReadyEvent 2")
		return
	}

	if mr.Event != signalerRequestEvent && mr.Event != signalerReadyEvent {
